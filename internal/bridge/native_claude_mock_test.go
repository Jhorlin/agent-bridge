...
func TestNativeClaudeAgentAndHookWithLocalEndpoint(t *testing.T) {
	for _, event := range []string{"SessionStart", "UserPromptSubmit", "Stop"} {
		t.Run(event, func(t *testing.T) {
			...
			if event == "Stop" {
				if _, ok := payload["stop_hook_active"].(bool); !ok {
					t.Fatal("missing stop recursion flag")
				}
			}
			if payload["hook_event_name"] != event || payload["cwd"] != f.dir || (event == "SessionStart" && payload["source"] != "startup") || (event == "UserPromptSubmit" && payload["prompt"] != "Return fixture complete.") {
