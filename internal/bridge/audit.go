...
func Audit(c Config) AuditReport {
	...
	for _, r := range resources {
		...
		case "plugin-directory":
			row.Actions = append(row.Actions, "Review skill behavior and install or refresh separately in each host; bounded conventional compatibility-layout hooks, commands and allowlisted MCP are bridged. Command names differ after Codex migration; arguments and bundled agents remain unsupported.")
		case "agent-file":
