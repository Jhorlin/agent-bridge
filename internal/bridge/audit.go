...
func Audit(c Config) AuditReport {
	...
	for _, r := range resources {
		...
		case "file-guard-config":
			row.Actions = append(row.Actions, "Only one explicitly allowlisted path-only guard reference maps Edit/Write/NotebookEdit to apply_patch through hook-file-guard. Unrelated hooks and Claude timeout remain local. Runtime failures deny more strictly; shell writes, notebook semantics and arbitrary content-sensitive guards are not equivalent. Review script changes and trust new or changed native hook definitions separately; synchronization does not activate hooks or install dependencies.")
		}
