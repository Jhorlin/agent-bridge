...
| Skill metadata and invocation | Claude frontmatter/invocation controls; Codex skill metadata and `agents/openai.yaml` | Partial: strict name/description plus separately opted-in implicit-invocation policy; exact body bytes | Other metadata, UI, dependencies, tools and isolation are not mapped. [Strict mode](skill-metadata.md), [paired invocation policy](skill-invocation.md) |
...
| --- | --- | --- |
| MCP stdio / HTTP definitions | Partial: allowlisted servers, supported command/arguments, HTTP URL; JSON ↔ TOML | Source builds merge independent server edits with recorded baselines; alpha remains whole-set. Nonempty cwd rejected because Claude ignores it. No SSE or remote executor mapping. [T3](#test-evidence) |
...
| MCP formatting | Partial: semantic comparison, unrelated value preservation and verified scalar text patches, including same-length arrays/inline tables | New/removed keys, array-length and unsupported shape changes still reformat. Explicit `allowReformat` remains required. [Limits](mcp-formatting.md), [T3](#test-evidence) |
...
| Hooks | Partial: startup SessionStart, plus prompt/Stop and exact-Bash pre/post events in source builds; bundled hooks require compatibility layout | Explicit timeout; absolute executable or bounded [quoted package-relative executable](plugin-runtime-paths.md). Native denial, failure, timeout and trust tests, not arbitrary policy equivalence. [Tool hooks](tool-hooks.md) |
...
| Conflict / drift handling | Baselines, reviewed side/history choices, supporting-file rename/delete and reviewed historical undo; [whole-resource retirement preserving files](retirement.md) | No last-writer-wins; retirement does not delete native entry points or uninstall plugins. [Drift workflow](conflict-resolution.md), [file changes](supporting-file-changes.md) |
...
| T2 | [Engine tests](../internal/bridge/bridge_test.go): `TestSkillBytesAndExecutables`, `TestSkillIndependentEditsAndAdditions`, `TestSkillPortabilityRequired`; [strict metadata tests](../internal/bridge/skill_test.go), [policy tests](../internal/bridge/skill_invocation_test.go), and [native policy matrix](../internal/bridge/native_skill_policy_test.go) | Rich metadata/dependencies, other execution contexts and real-model skill behavior |
...
| T4 | [Adapter tests](../internal/bridge/features_test.go): `TestPluginPackageTranslationAndReverseEdit`, `TestPluginPortableCodexLayout`, `TestPluginUnsupportedComponentsBlockAllWrites`, `TestPluginFailureRecoveryAndNoInstallSideEffects`; [agent exports](../internal/bridge/plugin_agents_test.go) and [native export loading](../internal/bridge/native_plugin_agents_test.go) | Arbitrary plugin execution, production refresh, UI/apps and direct bundled Codex agents |
...
| T7 | [Engine tests](../internal/bridge/bridge_test.go): `TestPartialFailureRollback`, `TestProcessInterruptionRecovery`, `TestLaterEditBlocksRecovery`, `TestRecoveryTargetValidation`; [file-change undo](../internal/bridge/file_change_history_test.go) and [corrupt-journal guards](../internal/bridge/file_change_validation_test.go) | Power loss and adversarial filesystem races |
...
2. **Instruction overlays and bounded skill metadata — implemented.** Common sections preserve native overlays. Strict skill mode reconciles name/description while preserving body bytes; a separate opt-in maps implicit-invocation policy with paired Codex sidecar recovery. Other rich metadata remains unsupported. See [strict skills](skill-metadata.md) and [policy scope](skill-invocation.md).
...
4. **Broader MCP — partially implemented.** Source builds merge independent selected servers, retain opted-in Codex-local policies, and preserve surrounding formatting for supported scalar/container-leaf patches. Bundled compatibility-layout MCP is allowlisted and native-tested. Structural formatting preservation and plugin-root relocation remain incomplete; see [phase-two evidence](phase-two.md).
5. **Custom agents — bounded native loading evidence.** Common fields translate; optional native settings remain host-local. Explicit plugin-agent exports provide a standalone Codex alternative, not equivalent package lifecycle or model/permission semantics. Both synthetic loading tests and an inert licensed upstream fixture are retained. See [agent export limits](plugin-agent-exports.md).
6. **Hooks and plugin lifecycle — bounded native tests implemented.** Startup, prompt/Stop and exact-Bash pre/post fixtures cover selected payloads, denial, failures, timeouts and trust behavior. Disposable install/remove/reinstall checks do not authorize or implement production refresh. Other events, arbitrary scripts and output policies remain unmapped. [Tool-hook limits](tool-hooks.md).
7. **Global onboarding and process lifecycle — partial.** Reviewed profile creation/enrollment, explicit ownership coordination, watcher/process handling, macOS services and experimental Linux services are implemented. Linux CI exercises an isolated real user manager, restart and same-source replacement; separate [pinned-alpha upgrade tests](upgrading.md) verify legacy data/journals. [Resource retirement](retirement.md) preserves files and baselines. Automatic upgrades and reboot/login remain outside current support. Service removal preserves native files, state and backups.
