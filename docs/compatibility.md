...
7. **Global onboarding and process lifecycle — partial.** Read-only discovery, empty-profile creation, optional common coordination, stable-input debounce, read-error retries, and process-level SIGINT/SIGTERM/restart tests are implemented. OS service installation/status/uninstall and automatic resource ownership coordination remain future work. Native files and backups must survive any future service removal.
