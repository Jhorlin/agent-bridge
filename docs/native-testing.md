...
Configuration readers and package validators are an initial integration layer. MCP startup/tool calls, Codex skill/plugin discovery, host instruction loading, and model behavior still need separate coverage. Machine-managed policies may influence native CLIs even with disposable user directories; this is configuration isolation, not an OS security sandbox.
